...
exports.process = function(srcFile, dstFolder, templates) {
  ...
  if (fileExt === '.md' || fileExt === '.markdown' ||
    ...
    fs.readFile(srcFile, 'utf8', function(err, data) {
      ...
      if (extract.attributes.layout && templates[extract.attributes.layout]) {
        // use layout if defined in metadata
        var render = templates[extract.attributes.layout].render({
          'content': html
        });
